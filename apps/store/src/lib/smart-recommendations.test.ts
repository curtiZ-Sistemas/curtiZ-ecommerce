...
describe("recomendações inteligentes", () => {
  it("prioriza o perfil consentido e só busca contexto quando faltam produtos", async () => {
    ...
    expect(JSON.parse(String(calls[0]?.init?.body))).toMatchObject({ source: "personalized", seed: "strass" });
  ...
  it("exclui o produto atual e aplica categoria e preço no contexto do produto", async () => {
    ...
    const primary = JSON.parse(String(calls[0]?.init?.body)) as Record<string, unknown>;
    expect(primary).toMatchObject({ category: "Sandálias", priceMin: 3900, priceMax: 8100 });
    expect(calls[1]?.init?.method).toBe("POST");
    expect(JSON.parse(String(calls[1]?.init?.body))).toMatchObject({ source: "price_range" });
  });
