...
export default async function ModelProductsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="section container"><div className="section-heading"><div><p className="eyebrow">Modelo</p><h1>{title}</h1></div></div>{products.length ? <div className="product-grid">{products.map((product, index) => <ProductCard product={product} priority={index === 0} key={storefrontItemKey(product)} />)}</div> : <div className="empty-state" role="status"><h2>Nenhum produto disponível</h2><p>Este modelo não possui produtos ativos e com estoque no momento.</p></div>}</main>;
}
