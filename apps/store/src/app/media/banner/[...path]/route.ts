...
import { isSafeBannerStoragePath } from "../../../../lib/banner-media";
...
export function isValidBannerObjectPath(segments: string[]): boolean {
  ...
  return isSafeBannerStoragePath(segments.join("/"));
}
...
export async function GET(
  ...
  if (!Array.isArray(segments) || !isValidBannerObjectPath(segments)) {
    return new NextResponse("Not found", { status: 404, headers: { "cache-control": "no-store" } });
  }

  const origin = configuredSupabaseOrigin();
  if (!origin) return unavailable(503, "Banner unavailable");

  const encodedPath = segments.map(encodeURIComponent).join("/");
  const sourceUrl = `${origin}/storage/v1/object/public/catalog-public/${encodedPath}`;
  let upstream: Response;
  try {
    upstream = await fetch(sourceUrl, { redirect: "manual" });
  } catch {
    return unavailable(502, "Banner unavailable");
  }
  if (!upstream.ok || upstream.status >= 300) return unavailable(upstream.status === 404 ? 404 : 502, "Banner unavailable");
  ...
  if (contentLength > MAX_BANNER_BYTES) return unavailable(413, "Banner too large");
