...
export async function POST(request: NextRequest) {
  ...
  if (result.error) {
    ...
    return NextResponse.json({ message: forbidden ? "Sua permissão não permite esta alteração." : "A alteração não é permitida para o estado atual." }, { status: forbidden ? 403 : result.error.code === "P0002" ? 404 : 409, headers: managerNoStore });
  }
