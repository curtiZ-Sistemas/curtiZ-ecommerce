-- Preserve public catalog contracts; match available variants and accent-insensitive terms.
...
as $$
  with product_data as (
    ...
    where (
      ...
      or to_tsvector('simple', extensions.unaccent(concat_ws(' ', item.base_name, item.description, item.category,
          item.variant_color, item.variant_size, item.sku)))
        @@ websearch_to_tsquery('simple', extensions.unaccent(trim(p_query)))
      or extensions.unaccent(concat_ws(' ', item.base_name, item.variant_color, item.variant_size, item.sku))
  ...
  filtered as (
    ...
    where (cardinality(p_colors) = 0 or item.colors && p_colors)
      ...
      -- Aggregated fallback cards must match ONE available size/color combination.
      and (
        (cardinality(p_colors) = 0 and cardinality(p_sizes) = 0)
        or exists (
          select 1
          from public.product_variants matched
          join public.inventory stock on stock.variant_id = matched.id
          where matched.product_id = item.product_id and matched.active
            and (item.variant_id is null or matched.id = item.variant_id)
            and (cardinality(p_colors) = 0 or matched.color_name = any(p_colors))
            and (cardinality(p_sizes) = 0 or matched.size = any(p_sizes))
            and stock.available_quantity - stock.reserved_quantity > 0
        )
      )
