...
create policy "homepage media managers update own" on storage.objects for update to authenticated using(bucket_id='homepage-public' and owner_id=auth.uid()::text and private.has_permission('homepage.media.manage')) with check(bucket_id='homepage-public' and owner_id=auth.uid()::text and private.has_permission('homepage.media.manage'));
create policy "homepage media managers remove own orphan" on storage.objects for delete to authenticated using(bucket_id='homepage-public' and owner_id=auth.uid()::text and private.has_permission('homepage.media.manage') and not exists(select 1 from public.home_section_item_media media where media.storage_path=name));
